...
export function SegmentedControl<T extends string>({ value, options, onChange, className }: SegmentedControlProps<T>) {
  return (
    <div className={clsx('flex rounded-md bg-surface-soft p-1', className)} role="tablist">
      {options.map((opt) => (
        <button
          key={opt.value}
          role="tab"
          ...
          className={clsx(
            ...
            value === opt.value ? 'bg-surface-raised text-forest shadow-glass' : 'text-ink-muted hover:text-ink-text'
          )}
        >
          {opt.label}
        </button>
