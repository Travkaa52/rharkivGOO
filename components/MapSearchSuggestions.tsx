...
export function MapSearchSuggestions({ stops, routes, onStopSelect, onRouteSelect }: MapSearchSuggestionsProps) {
  if (stops.length === 0 && routes.length === 0) {
    return (
      <div className="animate-slide-up rounded-xl2 border border-ink-border bg-ink-surface/95 p-4 text-center shadow-glass-lg backdrop-blur-xs">
        <p className="font-body text-sm text-white/50">Нічого не знайдено</p>
      </div>
    );
  }

  return (
    <div className="max-h-[60vh] animate-slide-up overflow-y-auto rounded-xl2 border border-ink-border bg-ink-surface/95 p-1.5 shadow-glass-lg backdrop-blur-xs">
      {routes.length > 0 && (
        <div className="mb-1">
          <p className="px-2.5 pb-1 pt-1.5 font-display text-[10px] font-bold uppercase tracking-wide text-white/40">
            Маршрути
          </p>
          {routes.map((route) => (
            <button
        ...
        <div>
          ...
          {stops.map((stop) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1 truncate font-body text-sm font-medium text-white">{stop.name}</span>
            </button>
          ))}
        </div>
      )}
