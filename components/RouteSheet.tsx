...
export function RouteSheet({ route, onClose, onStopSelect }: RouteSheetProps) {
  ...
  return (
    <div className="flex max-h-[70vh] flex-col overflow-hidden rounded-xl2 border border-ink-border bg-ink-surface/95 shadow-glass-lg backdrop-blur-xs">
      <div className="flex items-start gap-3 border-b border-ink-border p-3">
        <div
        ...
        {stops.map((stop, i) => (
          <li key={`${stop.id}-${i}`}>
            ...
            >
              ...
              <span className="truncate py-1 font-body text-sm text-white">{stop.name}</span>
            </button>
          </li>
        ))}
      </ul>
