...
export function MapView({
  ...
  useEffect(() => {
    ...
    // Явно вимикаємо touch-action браузера на самому canvas (дублює CSS-правило
    // в index.css) — деякі мобільні WebView ігнорують touch-action заданий лише
    // класом, якщо canvas ще не отримав його до першого touchstart.
    map.getCanvas().style.touchAction = 'none';

    // Відновлення після втрати WebGL-контексту (типова причина чорного екрана
    // під час активного pinch-zoom на слабких/мобільних GPU) — без цього карта
    // лишається чорною назавжди, з цим вона сама перемальовується.
    ...
    // Підстраховка: після завершення будь-якого pinch/zoom жесту форсуємо
    // resize() на наступному кадрі, щоб canvas.width/height точно збігались
    // з поточним devicePixelRatio (актуально, якщо ОС/браузер підмінює масштаб
    // сторінки під час жесту).
    const handleZoomEnd = () => requestAnimationFrame(() => map.resize());
    ...
    <div className="relative h-full w-full overflow-hidden">
      ...
      {userPosition && (
        <button
          ...
          className="absolute bottom-24 right-4 flex h-12 w-12 items-center justify-center rounded-full bg-ink-surface/90 shadow-glass-lg backdrop-blur-xs transition hover:scale-105 active:scale-95"
        >
