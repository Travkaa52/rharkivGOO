...
export function TransportLayersPanel() {
  ...
  return (
    <div className="pointer-events-auto absolute bottom-24 left-4 z-30 flex flex-col items-start gap-2">
      {open && (
        <div className="w-56 animate-slide-up rounded-xl2 border border-ink-border bg-ink-surface/95 p-3 shadow-glass-lg backdrop-blur-xs">
          <div className="mb-2 flex items-center justify-between">
            <p className="font-display text-xs font-bold uppercase tracking-wide text-white/60">На карті</p>
            <button
              type="button"
              onClick={showAll}
              disabled={allVisible}
              className={clsx(
                ...
                allVisible ? 'text-white/30' : 'text-mint hover:underline'
              )}
            >
              Показати все
            </button>
            ...
            {KIND_ORDER.map((kind) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 font-body text-sm font-medium text-white">{KIND_LABELS_UK[kind]}</span>
                  <span
                    className={clsx(
                      'relative h-5 w-9 shrink-0 rounded-full transition-colors',
                      active ? 'bg-forest' : 'bg-white/10'
                    )}
                  >
                    <span
                      className={clsx(
          ...
          <div className="mt-2 border-t border-ink-border pt-2">
            ...
            >
              <span className="flex h-5 w-5 items-center justify-center rounded-full border-2 border-forest bg-white text-[9px] font-bold text-mint">
                •
              </span>
              <span className="flex-1 font-body text-sm font-medium text-white">Зупинки</span>
              <span
                className={clsx(
                  'relative h-5 w-9 shrink-0 rounded-full transition-colors',
                  showStops ? 'bg-forest' : 'bg-white/10'
                )}
              >
                <span
                  className={clsx(
      ...
      <button
        ...
        className={clsx(
          ...
          open ? 'bg-forest text-white' : 'bg-ink-surface/90 text-mint'
        )}
      >
