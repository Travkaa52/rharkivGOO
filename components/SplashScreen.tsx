...
const STATUS_STEPS = ['Підключення до мережі…', 'Завантаження карти Харкова…', 'Синхронізація розкладів…', 'Готово'];
...
export function SplashScreen({ leaving, onLeaveEnd }: SplashScreenProps) {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      setStep((s) => Math.min(s + 1, STATUS_STEPS.length - 2));
    }, 700);
  ...
  return (
    ...
    >
      ...
      {/* Індикатор завантаження + статус — транслюється, а не змінює width, тому не тригерить layout */}
      <div className="absolute bottom-[max(2.5rem,env(safe-area-inset-bottom))] flex flex-col items-center gap-3">
        <p className="text-caption text-white/50">{STATUS_STEPS[step]}</p>
